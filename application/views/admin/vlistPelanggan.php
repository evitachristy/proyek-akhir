				...
				<tbody>
					<?php foreach ($muser as $key ) : ?>
									<tr>
										...
										<td><?=$key->email ?></td>
										<td><?=$key->notelp ?></td>
										<?= "<td>". anchor('cadmin/editPelanggan/'.$key->id_pelanggan,'<button type="button" class="btn btn-primary">Edit</button>'); ?>
										<?= "<td>". anchor('cadmin/delete/'.$key->id_pelanggan,'<button type="button" class="btn btn-danger">Hapus</button>'); ?>
									</tr>
									<?php endforeach; ?>
